...
public class FrictionMotorSim {
  ...
  public FrictionMotorSim(
      DCMotor motor,
      MomentOfInertia moi,
      double frictionStatic,
      double frictionDynamic,
      boolean brakeMode) {
  ...
  public void update(Time dt) {
    ...
    AngularAcceleration acceleration =
        Units.RadiansPerSecondPerSecond.of(
            netTorque.in(Units.NewtonMeters) / moi.in(Units.KilogramSquareMeters));
  ...
  private Voltage getSmartVoltage(AngularVelocity speed, Current smartCurrent) {
    return Units.Volts.of(
        smartCurrent.in(Units.Amps) * motor.rOhms
            + speed.in(Units.RadiansPerSecond) / motor.KvRadPerSecPerVolt);
  }
  ...
  public void updateInputs(MotorIOInputs inputs) {
    ...
    inputs.output =
        inputs.outputVoltage.divide(BatterySimManager.getBatteryVoltage()).baseUnitMagnitude();
